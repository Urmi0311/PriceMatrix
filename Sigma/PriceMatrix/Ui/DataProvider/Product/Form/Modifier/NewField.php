    ...
    public function modifyData(array $data)
    {
        ...
        if ($productId) {
            ...
            if ($priceMatrix->getId()) {
                for ($i = 1; $i <= 5; $i++) {
                    $data[$productId]['product']['custom_fieldset']['base_price_container']['display_base_price_' . $i] = $priceMatrix->getData('display_base_price_' . $i);
                    $data[$productId]['product']['custom_fieldset']['qty_container']['display_qty_' . $i] = $priceMatrix->getData('display_qty_' . $i);
                }
            }

            $logger->info("hello " . print_r($data, true));
        }
